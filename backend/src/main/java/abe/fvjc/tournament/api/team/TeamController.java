...
class TeamController {
    ...
    public List<TeamDto> getAll(@PathVariable UUID tournamentId) {
        final var teams = teamService.findAllByTournamentId(tournamentId);
        return toTeamDtos(teams);
    }
    ...
    public TeamDto update(
        ...
        final var team = teamService.updateTeam(tournamentId, teamId, request);
    ...
    public TeamDto updatePaid(
            @PathVariable UUID tournamentId,
            @PathVariable UUID teamId,
            @RequestBody @Valid TeamPaymentRequestDto requestDto) {
        final var team = teamService.markPaid(teamId, requestDto.getPaid());
        return toTeamDto(team);
    }
}
